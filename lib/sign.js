...
// 生成包含签名的和过期时间的对象
function createTokenObj(token, params) {
    // 按照排序的键值组合
    var str = ''
    var keys = []
    ...
    // 生成签名
    params.sign = crypto.createHash('md5').update(str + token).digest('hex')
...
exports.create = function (token, expire, params) {
    ...
    var obj = createTokenObj(token, params)
